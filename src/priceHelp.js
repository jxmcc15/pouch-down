...
// transport as coach.js: the proxy this person owns when one is configured and
// this device has a token, otherwise the direct call with the session key
// (sessionKey.js), which is never stored on the device and never ships.
//
// Deliberately isolated: everything about *how* the call is addressed lives in
// proxyConfig.js, so this file only ever describes what it wants to ask.
import { coachTransport, authErrorFor } from './proxyConfig.js';

// Same model id as coach.js, which has been making this exact call in
// production in this app since July. If a live call ever comes back with a
// model-not-found error, the one-line fix is to drop the date and use the
// undated alias: 'claude-haiku-4-5'.
...
//   'no-key'           no proxy configured and no session key
//   'no-device-token'  a proxy is configured, this device isn't connected yet
//   'bad-key'          401 on the direct call — the key was rejected
//   'bad-device-token' 401 from the proxy — the device token was rejected
...
// error and falls into the sheet's catch-all copy. Whichever credential is in
// play travels in a header only — never in a URL, a log, or an error message.
